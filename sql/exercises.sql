...
# ---------------------------------------------------------#
FROM film (SELECT col as title, column 2 as description, col 3 as rental_duration, col 4 as rental_rate, col 5 as total_rental_cost)
...
SELECT * FROM film WHERE movie length >3;
...
SELECT * FROM payments WHERE DATE >= '2005-27-05' date;

# 3c. Select the primary key, amount, and payment date columns from the payment table for payments made on 05/27/2005.
SELECT primary_key, amount, payment_date FROM payment where DATE = "2005-27-05" date;
...
email, store_id, active, username, last_update FROM staff WHERE password IS NOT NULL; 

# 3h. Select all columns minus the password column from the staff table for rows that do not contain a password.
SELECT staff_id, first_name, last_name, address_id, picture, email, store_id, active, 
username, last_update FROM staff WHERE password IS NULL;
